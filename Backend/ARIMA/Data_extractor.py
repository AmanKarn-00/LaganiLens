from pymongo import MongoClient
import pandas as pd
import numpy as np
from statsmodels.tsa.arima.model import ARIMA
from datetime import datetime

# MongoDB connection
client = MongoClient("mongodb://localhost:27017")
db = client["laganiLens"]

# ✅ CORRECTED COLLECTION NAME
# Your Mongoose model saves to 'historicaldatas', not 'nepsestocks'
collection = db["historicaldatas"] 
pred_collection = db["predictedStocks"]

# Clear existing predictions
pred_collection.delete_many({}) 

# Get all unique stock symbols
symbols = collection.distinct("symbol")

# Number of days to forecast
forecast_days = 30

print(f"Found {len(symbols)} stocks. Starting analysis...")

for symbol in symbols:
    print(f"Processing {symbol}...")
    
    # Fetch data (sorted by date inside logic)
    cursor = collection.find(
        {"symbol": symbol},
        {"_id": 0, "date": 1, "close": 1}
    )
    
    df = pd.DataFrame(list(cursor))
    if df.empty:
        print(f"No data for {symbol}, skipping.")
        continue

    # Prepare the data
    try:
        df["date"] = pd.to_datetime(df["date"])
        df["close"] = pd.to_numeric(df["close"], errors="coerce")
        df = df.dropna()
        df = df.sort_values("date")
        df.set_index("date", inplace=True)
        
        # Ensure daily frequency and fill gaps
        df = df.groupby(df.index).last() 
        df = df.asfreq("D")              
        df["close"] = df["close"].ffill()
        
        # Calculate Log Difference
        df["log"] = np.log(df["close"])
        df["diff"] = df["log"].diff()
        df = df.dropna()
        
        series = df["diff"]

        # Fit ARIMA model
        model = ARIMA(series, order=(5,1,0))
        model_fit = model.fit()

        # Forecast
        forecast = model_fit.forecast(steps=forecast_days)
        last_price = df["close"].iloc[-1]
        
        # Convert log returns back to price
        future_prices = np.exp(forecast.cumsum()) * last_price

        # ✅ NEW: Get last 30 days of HISTORY for the graph
        # This allows the frontend to draw a continuous line from past to future
        recent_history_df = df.iloc[-30:] 
        history_data = []
        
        for date_idx, row in recent_history_df.iterrows():
            history_data.append({
                "date": date_idx.strftime("%Y-%m-%d"), 
                "price": float(row["close"])
            })

        # Prepare document for MongoDB
        doc = {
            "symbol": symbol,
            "last_price": float(last_price),
            "forecast_days": forecast_days,
            "predicted_prices": [float(p) for p in future_prices],
            "history": history_data, # <--- Added history field
            "generated_at": datetime.utcnow()
        }

        # Insert new document
        pred_collection.insert_one(doc)
    
    except Exception as e:
        print(f"Error processing {symbol}: {e}")
        continue

print("✅ All predictions stored successfully.")